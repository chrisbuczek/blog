...
JWT's purpose is to securely transmit information between parties as json objects. It allows us to authenticate users (process of verifying a user's identity) and authorize them to perform some actions (process of verifing user's access) JWT's are structed from three parts:
...
should never travel in a JWT payload** (unless it's encrypted).

## Generating/signing the token

There are two ways of signing a token: with a symetric key or with an asymetric key.
Before genereting the token we should create a secret string key that will be stored in environment variables. In the tokenGenerate.ts example I named it JWT_SECRET_KEY.
...
When we generate the token we have to decide for how long the token will be valid. In the example above we are making the token valid for 1 hour. This will be our <span className="font-bold">access token</span>. After 1 hour
the client will have to ask the server for a new token - <span className="font-bold">refresh token</span>. It will allow user to keep being logged in.
